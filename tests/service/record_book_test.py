...
@patch('src.service.record_book.UserService')
@patch('src.service.record_book.RecordBookRepository')
def test_should_delete_record_from_record_book_given_record_book_id_and_username_and_record_id(
        mocked_record_book_repository, mocked_user_service):
    record_book_id = str(uuid.uuid4())
    ...
    with mocked_session_class(mocked_engine) as session:
        record_book_service = RecordBookService(session)
        record_book_service.delete_record(record_book_id, username, record_id)
        mocked_user_service_instance.fetch_user.assert_called_with(username)
        mocked_record_book_repository_instance.fetch_record_book.assert_called_once_with(record_book_id,
                                                                                         mock_model_user().id)
        mocked_record_book_repository_instance.update_record_book.assert_called_once()
...
@patch('src.service.record_book.UserService')
@patch('src.service.record_book.RecordBookRepository')
def test_should_delete_record_book_given_record_book_id_and_username(mocked_record_book_repository,
                                                                     mocked_user_service):
    record_book_id = str(uuid.uuid4())
    username = "test_username"
    mocked_session_class = MagicMock(spec=Session)
    ...
    with mocked_session_class(mocked_engine) as session:
        record_book_service = RecordBookService(session)
        record_book_service.delete_record_book(record_book_id, username)
        mocked_user_service_instance.fetch_user.assert_called_with(username)
        mocked_record_book_repository_instance.delete_record_book.assert_called_once()
